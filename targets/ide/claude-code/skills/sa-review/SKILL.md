---
name: sa-review
description: "Review implementation against architectural intent and produce findings. Use when the user wants to check whether what was built matches the architecture, identify drift, or assess architectural risks."
---
...
For must-fix and should-fix findings, create issue files in `.workflow/issues/backlog/`. Run `.claude/scripts/next-issue-number.sh` for the next available number.
...
Run `.claude/scripts/update-issues-list.sh` after creating issues.
