...
For each regression failure, run `.claude/scripts/next-issue-number.sh` to get the next available issue number.
...
After creating issue files, run `.claude/scripts/update-issues-list.sh` to regenerate the issues list.
...
- The regression report feeds into PM's `/pm-postmortem` for the milestone
