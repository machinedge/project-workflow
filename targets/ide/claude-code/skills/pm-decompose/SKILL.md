...
Run `.claude/scripts/next-issue-number.sh` to get the next available issue number. Start numbering new issues from that number.
...
2. Run `.claude/scripts/update-issues-list.sh` to regenerate `.workflow/issues/issues-list.md`.
