...
Run `.cursor/scripts/next-session-number.sh devops` to atomically claim the next session number. The script creates a placeholder file to prevent concurrent sessions from claiming the same number. It outputs the claimed number (e.g., "03").
...
Save to `.workflow/handoff-notes/devops/session-NN.md`:
...
**Handoff note:** `.workflow/handoff-notes/devops/session-NN.md`
...
If anything was learned during this session (a gotcha, an environment quirk, a deployment pattern), add it to `.workflow/lessons-log.md`.
