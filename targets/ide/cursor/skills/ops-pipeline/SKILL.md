...
If CI config generation is needed, that's an SWE task — create an issue file for it in `issues/backlog/`. Run `.cursor/scripts/next-issue-number.sh` for the next number.
