...
- "Project: [1 sentence]"
- "Last SA session: [1 sentence from handoff, or 'First SA session']"
...
- If the task requires evaluating technology options, do the evaluation here (same rigor as the `sa-research` skill)
...
Then ask the user if they'd like to proceed to the `sa-handoff` skill to close out the session.
