...
### Standalone Mode (Experts in Cursor or Claude Code)

Install all experts into a project:

```bash
# Both editors (Cursor + Claude Code)
...
Then open the project in your editor and run `/pm-interview` to start a new project, or `/team-status` to see the project health summary.

Or install via the Claude Desktop / Cowork skill:
...
These definitions are platform-agnostic. Each IDE target (Cursor, Claude Code) has its own platform-native implementation in `targets/ide/`. The install script copies pre-built files directly — no translation step. **OpenClaw** uses its own deployment mechanism for team mode.
...
After updating the toolkit repo, use the sync command to check for drift and apply updates to installed projects:
...
# Compare Cursor and Claude Code implementations (maintainers)
./tools/sync.sh diff
```
...
│   │   ├── cursor/                 ← Cursor platform-native files
│   │   └── claude-code/            ← Claude Code platform-native files
