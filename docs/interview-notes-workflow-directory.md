...
- **`.workflow/`** — Managed workflow artifacts (agent memory system). Contains:
...
2. **Future data portability.** Consolidating managed artifacts under `.workflow/` creates a clear boundary — a defined surface for what could eventually be persisted to a database or external store.

## Scope

### In

- New `.workflow/` directory structure created on fresh install
- Migration logic in the install script: detect old layout (`docs/handoff-notes/`, `docs/interview-notes-*`, `docs/lessons-log.md`, `docs/research-*`, `issues/`), move files to `.workflow/`
...
- `.workflow/` is NOT auto-added to `.gitignore` — left to the user
- `agent-reference.md` stays in `docs/` (project-specific)
- `lessons-log.md` moves to `.workflow/`
...
- Fresh install into a new project creates `.workflow/` structure from the start
- Installing over an existing project (old structure) migrates all artifacts into `.workflow/` without data loss
