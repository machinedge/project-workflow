...
Updated all 36 SKILL.md files (18 per platform) in `targets/ide/cursor/skills/` and `targets/ide/claude-code/skills/` to replace old artifact paths with `.workflow/` equivalents per ADR-011. This completes step 3 (skills) of the M13 implementation order for both platforms.

## Acceptance Criteria Status

### swe-feature-066 (Cursor)
- [x] PM skills (pm-decompose, pm-handoff, pm-postmortem, pm-update-plan, pm-vision) reference `.workflow/` for handoff notes, interview notes, issues, and lessons-log
- [x] SWE skill (swe-handoff) references `.workflow/` for handoff notes, issues, and lessons-log
- [x] QA skills (qa-bug-triage, qa-handoff, qa-regression, qa-review, qa-test-plan) reference `.workflow/` for handoff notes, issues, and lessons-log
- [x] DevOps skills (ops-handoff, ops-pipeline, ops-release-plan) reference `.workflow/` for handoff notes, issues, and lessons-log
- [x] SA skills (sa-design, sa-handoff, sa-review) reference `.workflow/` for handoff notes, interview notes, issues, and lessons-log
- [x] team-status skill references `.workflow/` for issues and handoff notes
