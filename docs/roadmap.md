...
| M19 | [SDLC Boundary] Hard line on `docs/` — expert-authored specs/plans/drafts move to `.sdlc/`, `docs/` holds only user-facing docs (`guides/`, `README.md`, `project-brief.md`, `roadmap.md`); repoint every spec skill/role/runbook/accelerator to `.sdlc/`, make consumers fail loudly at the new path, and add a `migrate-sdlc` workflow that relocates an existing project's specs and verifies | Done | M18 | 3-5 (actual: 7 planned + 15-issue fix tail) |
...
M18 (Technical Writer) ──> M19 (SDLC Boundary: specs → `.sdlc/`, `docs/` user-facing only, `migrate-sdlc` workflow)
(M19 builds on the M13/M14 `.sdlc/` split + migration pattern, extending it from session artifacts to expert specs.)
...
| ~~[M19] Stale path references — specs are cross-referenced widely (roles, skills, both `team-*` runbooks, both accelerators, QA/SWE consumers); a miss means an expert reads/writes the wrong place~~ | ~~High~~ | ~~Medium~~ | **Resolved in M19:** the QA grep audit (qa-feature-094) reports zero stale `docs/<spec>.md` authoring references across `agents/`, both install scripts, and the brief. One miss slipped past the first repoint pass (skill files still pointing at `docs/`, swe-bug-095) and was caught and fixed. |
| ~~[M19] `migrate-sdlc` loses or clobbers user edits when relocating specs on an existing project~~ | ~~High~~ | ~~Low~~ | **Partly realized, then fixed:** the migration draft glob was over-broad and relocated user-owned `docs/*-draft.md` files (swe-bug-096, swe-bug-101) — both fixed. A residual edge remains open: the `docs/security/` named subfolder is still migrated wholesale, capturing any user content under that name (swe-bug-102, should-fix, deferred). The collision guard and idempotent re-run behavior both verified. |
| ~~[M19] `docs/architecture.md` has the widest consumer fan-out (SWE, QA, Sec, UX, TW, PM all read it) — easiest single file to miss~~ | ~~Medium~~ | ~~Medium~~ | **Resolved in M19:** the file now lives at `.sdlc/architecture.md`; the fail-loud directive on the consume step names the file and remedy when it is absent, verified against the widest-fan-out consumer. |
...
| 2026-06-19 | Postmortem: M19 ([SDLC Boundary]) marked Done. All 7 planned tasks delivered (repoint skills/roles/runbooks/accelerators/installer + `migrate-sdlc` workflow + QA audit). The QA grep audit (qa-feature-094) confirms zero stale `docs/<spec>.md` authoring references; specs now live in `.sdlc/`. The close-out review fan-out (Security/UX/Technical Writer/SWE lenses) filed a 15-issue fix tail (swe-bug-095/096/101/103-109, swe-techdebt-097-100) — almost all resolved. Two follow-ups remain open in backlog: the `docs/security/` wholesale-move edge (swe-bug-102, should-fix, deferred) and the deeper-reading drill-down feature (pm-feature-087, separate future feature). One process gap: the entire M19 change set is still uncommitted in the working tree even though swe-techdebt-099 (which required committing it) was marked done — commit before starting new work. |
| 2026-06-19 | Added [SDLC Boundary] milestone (M19) from interview notes (`.sdlc/interview-notes-sdlc-boundary.md`). Draws a hard line: expert-authored specs/plans/drafts (`architecture.md`, `security-requirements.md`, `ux-guidelines.md`, `documentation-plan.md`, `test-plan.md`, `pipeline.md`, `components.md`, `task-detail-standard.md`, drafts, and the `research/`/`security/`/`runbooks/` subfolders) move to `.sdlc/`; `docs/` keeps only user-facing material (`guides/`, `README.md`, `project-brief.md`, `roadmap.md`). Repoint every spec skill/role/runbook/accelerator to `.sdlc/`; consumers **fail loudly** at the new path; add a `migrate-sdlc` workflow that relocates an existing project's specs (idempotent) and verifies. Filed a backlog feature (`pm-feature-087`): let readers drill deeper than the user-facing docs. |
