# Test Workflow Toolkit — Setup (Windows)
...
Write-Host "Setting up test-workflow toolkit in: $Target (editor: $Editor)"
...
# ─────────────────────────────────────────────
# Cursor setup
# editor.md + YAML frontmatter → .cursor/rules/test-workflow-os.mdc
# ─────────────────────────────────────────────
...
if ($Editor -eq "cursor" -or $Editor -eq "both") {
    ...
    $frontmatter = @"
---
description: Test Workflow — multi-session test-workflow protocol
alwaysApply: true
---

"@
    $editorContent = Get-Content "$ScriptDir/editor.md" -Raw
    ($frontmatter + $editorContent) | Set-Content -Path "$Target/.cursor/rules/test-workflow-os.mdc" -Encoding UTF8
...
Write-Host "  3. Follow the workflow: /brief -> /plan -> /decompose -> /start"
Write-Host ""
