<!-- TEMPLATE: Decompose Command
     ...
     Replace plan.md with the plan file.
     Replace project-brief.md with the brief file.
     Replace milestone with your domain term (milestone, phase, etc.).
     Replace task with the GitHub label for tasks (e.g., "task", "analysis").
     Delete these comments when done.
-->
The user wants to break a milestone into individual tasks.

If the user specified a milestone: $ARGUMENTS

Read `docs/plan.md` and `docs/project-brief.md`.

If the user didn't specify which milestone, check the brief's "Current Status" section or ask them.

Break the milestone into session-sized tasks. Each task should be completable in a single `/start` session.
...
- [ ] [Another criterion — what can the user do or see?]
