...
As a developer using Cursor, I need all expert skills to reference `.workflow/` paths, so that skill execution reads and writes managed artifacts to the correct location.

## Description

Update all SKILL.md files in `targets/ide/cursor/skills/` to replace old path references with `.workflow/` equivalents. This covers ~18 skill files across PM, SWE, QA, DevOps, SA, and team-status skills.
