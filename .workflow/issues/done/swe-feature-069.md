...
As a developer using Claude Code, I need the expert commands (`/pm-start`, `/swe-start`, etc.) to reference `.workflow/` paths, so that session startup reads from and writes to the correct locations.

## Description

Update all command files in `targets/ide/claude-code/commands/` to replace old path references with `.workflow/` equivalents. Commands affected: `pm-start`, `pm-interview`, `pm-add-feature`, `swe-start`, `qa-start`, `sa-start`, `ops-start`, `ops-deploy`.
