...
1. **Line 31** — Top-level directory layout shows `issues/` as a standalone directory. Should show `.workflow/` structure instead.
2. **Line 346** — Cursor `project-os.mdc` description says `issues in `issues/``. Should be `.workflow/issues/`.
3. **Line 513** — Session-primer.sh behavior says `docs/handoff-notes/`. Should be `.workflow/handoff-notes/`.
4. **Line 537** — Script specifications table says output is `Regenerated `issues/issues-list.md``. Should be `.workflow/issues/issues-list.md`.

## Acceptance Criteria

- [ ] Line 31: Top-level directory layout updated to show `.workflow/` structure (add `.workflow/` with subdirs, remove standalone `issues/`)
- [ ] Line 346: `issues/` → `.workflow/issues/`
- [ ] Line 513: `docs/handoff-notes/` → `.workflow/handoff-notes/`
- [ ] Line 537: `issues/issues-list.md` → `.workflow/issues/issues-list.md`
