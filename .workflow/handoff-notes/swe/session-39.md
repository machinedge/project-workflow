...
Updated `targets/ide/install.sh` and `targets/ide/install.ps1` to create `.workflow/` directory structure instead of `docs/handoff-notes/` and top-level `issues/` on fresh installs. Added detection logic to distinguish fresh installs from existing old-structure installs. This completes M13 step 5 (install scripts).

## Acceptance Criteria Status
- [x] Fresh install creates `.workflow/handoff-notes/{pm,swe,qa,devops,system-architect}/`
- [x] Fresh install creates `.workflow/issues/{backlog,planned,in-progress,done}/`
- [x] Fresh install seeds `.workflow/lessons-log.md` (instead of `docs/lessons-log.md`)
...
| Already-migrated projects (have `.workflow/`, no old structure) treated as fresh path | `mkdir -p` is idempotent and `lessons-log.md` check prevents overwrite — safe to re-run. |
...
- `targets/ide/install.sh` — project structure section → `.workflow/`, old-structure detection, updated uninstall message
