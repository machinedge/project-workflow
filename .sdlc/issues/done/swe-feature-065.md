...
As a developer using Cursor, I need the expert commands (`/pm-start`, `/swe-start`, etc.) to reference `.sdlc/` paths, so that session startup reads from and writes to the correct locations.

## Description

Update all command files in `targets/ide/cursor/commands/` to replace old path references with `.sdlc/` equivalents. Commands affected: `pm-start`, `pm-interview`, `pm-add-feature`, `swe-start`, `qa-start`, `sa-start`, `ops-start`, `ops-deploy`.
...
**What was done:** Updated all 8 Cursor command files to replace old artifact paths (docs/handoff-notes/, docs/lessons-log.md, docs/interview-notes*, issues/) with .sdlc/ equivalents per ADR-011.
