...
End-to-end verification that the migration from old directory structure to `.sdlc/` is complete, correct, and doesn't break any expert workflow. This is the final gate before M14 is marked done.

## Scope

- Verify all artifacts are in `.sdlc/` and none remain in old locations
- Verify `docs/` contains only planning docs and user content
- Test that each expert type can start a session and interact with `.sdlc/` paths
...
- [ ] All managed artifacts confirmed in `.sdlc/`
...
- [ ] All 4 core scripts execute successfully against `.sdlc/` paths
- [ ] Re-running install.sh does not duplicate or error
- [ ] Any issues found are filed as bug issues in `.sdlc/issues/backlog/` (using new path)

## Notes

**Depends on:** swe-feature-078 (migration must be tested first)
**Inputs:** SWE handoff notes from migration tasks, all `.sdlc/` and `docs/` contents
