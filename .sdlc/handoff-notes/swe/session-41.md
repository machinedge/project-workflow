...
- [x] `.cursor/rules/`, `.cursor/commands/`, `.cursor/skills/`, `.cursor/scripts/` all reference `.sdlc/` paths
- [x] `.claude/roles/`, `.claude/commands/`, `.claude/skills/`, `.claude/scripts/`, `.claude/CLAUDE.md` all reference `.sdlc/` paths
- [x] `grep -r 'docs/handoff-notes|docs/lessons-log|docs/interview-notes' .cursor/ .claude/` returns zero matches
- [x] `grep -r 'issues/backlog|issues/planned|issues/in-progress|issues/done' .cursor/ .claude/` — all matches are `.sdlc/issues/` (no stale standalone `issues/` refs)
...
The reinstalled `next-session-number.sh` now looks in `.sdlc/handoff-notes/swe/` which is empty — this project's data hasn't migrated yet (M14). Session numbering will be off for installed scripts until migration. Not a bug — expected edge case documented in swe-feature-072's detection logic.
