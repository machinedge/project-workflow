...
Updated all 16 command files (8 per platform) in `targets/ide/cursor/commands/` and `targets/ide/claude-code/commands/` to replace old artifact paths with `.sdlc/` equivalents per ADR-011. This completes step 2 (commands) of the M13 implementation order for both platforms.

## Acceptance Criteria Status

### swe-feature-065 (Cursor)
- [x] All `/start` commands load handoff notes from `.sdlc/handoff-notes/<expert>/`
- [x] All `/start` commands reference `.sdlc/issues/` for issue discovery
- [x] `pm-interview.md` writes interview notes to `.sdlc/`
- [x] `pm-add-feature.md` writes interview notes to `.sdlc/`
- [x] `swe-start.md` and `sa-start.md` reference `.sdlc/` for lessons-log
