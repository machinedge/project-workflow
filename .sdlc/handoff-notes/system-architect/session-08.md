...
Designed the `.sdlc/` directory structure (ADR-011) with a complete path mapping table covering all managed artifacts that move from `docs/` and top-level `issues/` into `.sdlc/`. Conducted a systematic audit of ~90 files per platform to inventory every path reference that needs updating.
...
| Decision | Reasoning |
|----------|-----------|
| Flat `.sdlc/` structure (Option A) | Artifacts already have clear names; sub-categories add depth without value. The move is a prefix change, not a reorganization. |
