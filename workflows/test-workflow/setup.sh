...
echo "Setting up test-workflow toolkit in: $TARGET (editor: $EDITOR)"
...
# GUIDE: Add domain-specific template files here. Examples:
#
...
if [ "$EDITOR" = "cursor" ] || [ "$EDITOR" = "both" ]; then
    ...
    } > "$TARGET/.cursor/rules/test-workflow-os.mdc"
...
echo "  3. Follow the workflow: /brief → /plan → /decompose → /start"
echo ""
