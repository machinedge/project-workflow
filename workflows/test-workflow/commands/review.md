<!-- TEMPLATE: Review Command
     This command runs a "fresh-eyes" review. It MUST be run in a separate
...
Read the project brief (`docs/project-brief.md`), then read the GitHub issue(s) being reviewed.
...
- **Must Fix** — Incorrect behavior, broken acceptance criteria, or significant risk. Creates a GitHub issue.
- **Should Fix** — Quality concern, missing edge case, or inconsistency. Creates a GitHub issue.
- **Nit** — Style, naming, or minor improvement. Mentioned in the review summary but not a GitHub issue.

---

## Step 4: Create Issues for Findings

For each Must Fix and Should Fix finding, create a GitHub issue:

```bash
gh issue create \
  --title "[Review] [Short description of finding]" \
  --label "task" \
  --body "$(cat <<'EOF'
## User Story
