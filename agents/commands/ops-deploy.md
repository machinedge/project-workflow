...
- Scan `.sdlc/issues/backlog/`, `.sdlc/issues/planned/`, and `.sdlc/issues/in-progress/` for any issues with **Severity: must-fix** — these must be resolved
- Scan for any issues with **Severity: blocker** — these must be resolved
- Check that all issues for the milestone are in `.sdlc/issues/done/` (none remaining in other status folders)
...
**If any gate FAILS: STOP.** Report what's not met and what needs to happen to fix it. Do NOT proceed with deployment. Create issue files in `.sdlc/issues/backlog/` for any blocking items if they don't already exist. Run `.agents/scripts/next-issue-number.sh` for the next number, then run `.agents/scripts/update-issues-list.sh` after creating the issue.
...
If the deployment failed or was rolled back, create an issue file in `.sdlc/issues/backlog/`. Run `.agents/scripts/next-issue-number.sh` for the next number.

Create `.sdlc/issues/backlog/devops-bug-[number].md`:
...
Run `.agents/scripts/update-issues-list.sh` after creating the issue.
