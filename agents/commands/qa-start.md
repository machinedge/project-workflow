...
3. The task issue:
   - If user specified an issue (e.g. "qa-feature-005" or a description), find and read it from `.sdlc/issues/`
   - If not, check project brief's "Next task" field for the issue filename
   - If still unclear, scan `.sdlc/issues/planned/` and `.sdlc/issues/in-progress/` for QA tasks and ask
4. Most recent handoff note in `.sdlc/handoff-notes/qa/` (if any exist)
5. Relevant SWE handoff notes in `.sdlc/handoff-notes/swe/` — understand what was built and what changed
...
- "Project: [1 sentence]"
- "Last QA session: [1 sentence from handoff, or 'First QA session']"
...
For each issue found, run `.agents/scripts/next-issue-number.sh` to get the next available issue number.

Create an issue file in `.sdlc/issues/backlog/` for each finding with a clear title, description, and severity. Reference the source file and line numbers where applicable. Include reproduction steps or evidence.
