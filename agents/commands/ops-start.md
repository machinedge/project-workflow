...
4. The task issue:
   - If user specified an issue (e.g. "devops-feature-001" or a description), find and read it from `.sdlc/issues/`
   - If not, check project brief's "Next task" field for the issue filename
   - If still unclear, scan `.sdlc/issues/planned/` and `.sdlc/issues/in-progress/` for DevOps tasks and ask
5. Most recent handoff note in `.sdlc/handoff-notes/devops/` (if any exist)
6. Relevant SWE handoff notes in `.sdlc/handoff-notes/swe/` — understand what was built (what needs to be deployed)
