...
if (stage === 'enrich') {
  phase('Enrich')
  log(`Enriching ${milestone} across SA, Security, UX, Docs, QA, DevOps (parallel drafts).`)
  const lenses = [
    { key: 'architecture', skill: 'sa-design', out: '.sdlc/architecture.md', focus: 'component boundaries, interfaces, data flow, and contracts for this milestone' },
    { key: 'security', skill: 'sec-requirements', out: '.sdlc/security-requirements.md', focus: 'a threat model and verifiable security controls (SR-NNN)' },
    { key: 'ux', skill: 'ux-guidelines', out: '.sdlc/ux-guidelines.md', focus: 'user flows, interaction patterns, accessibility, content/error clarity, and CLI ergonomics (UX-NNN); if the milestone has no user-facing surface, say so and produce a minimal note rather than inventing UI' },
    { key: 'documentation', skill: 'doc-plan', out: '.sdlc/documentation-plan.md', focus: 'audiences (end user / operator / maintainer), the guides this milestone needs, and verifiable DOC-NNN requirements written for readers unfamiliar with the project; if the milestone changes nothing a user or operator touches, say so and produce a minimal note rather than inventing docs' },
    { key: 'test', skill: 'qa-test-plan', out: '.sdlc/test-plan.md', focus: 'test strategy, acceptance criteria, and explicit test cases' },
...
if (stage === 'compile') {
  ...
  const decomp = await agent(
    `Run pm-decompose in IMPLEMENTATION-READY mode for milestone "${milestone}". First read docs/roadmap.md, confirm "${milestone}" matches a real roadmap milestone, and return its canonical "M<N> — <title>" as resolvedMilestone so the gate can confirm the right target. Read and follow .agents/skills/pm-decompose/SKILL.md (its Modes + implementation-ready template) and .sdlc/task-detail-standard.md. The skeleton task issues for this milestone ALREADY EXIST under .sdlc/issues/backlog/ (created in the Plan phase) — DENSIFY EACH ONE IN PLACE to implementation-ready, reusing its already-assigned issue number. Do NOT create new duplicate issues. Inline the architecture contracts, SR-NNN security constraints, and test cases from the enrichment artifacts. Return the list of task files with their dependency order.`,
...
if (stage === 'review') {
  phase('Review')
  log(`Running close-out gates for ${milestone} (QA, SA, Security, UX, Docs, regression) in parallel.`)
  const gates = [
    { key: 'qa', skill: 'qa-review', focus: 'code vs. intent and the test plan' },
    { key: 'architecture', skill: 'sa-review', focus: 'conformance to .sdlc/architecture.md. Read .sdlc/architecture.md; if it is absent, STOP and report: "architecture.md not found at .sdlc/architecture.md. Produce it with sa-design, or run migrate-sdlc for an existing project." Do not proceed — architecture is required for any implementation milestone' },
    ...
    { key: 'documentation', skill: 'doc-review', focus: 'DOC-NNN conformance: guides accurate vs. what shipped, complete, and followable by an unfamiliar reader; skip if no user/operator surface changed' },
...
if (stage === 'postmortem') {
  ...
  const pm = await agent(
    `Run the pm-postmortem skill for milestone "${milestone}". First read docs/roadmap.md, confirm "${milestone}" matches a real roadmap milestone, and return its canonical "M<N> — <title>" as resolvedMilestone. Read and follow EVERY step of .agents/skills/pm-postmortem/SKILL.md exactly — capture ALL of it. That means: load the full context it lists (docs/project-brief.md, docs/roadmap.md, ALL handoff notes under every .sdlc/handoff-notes/<expert>/ directory, .sdlc/lessons-log.md, .sdlc/test-plan.md and .sdlc/release-plan.md if they exist, and the milestone's issues across all .sdlc/issues/ subdirectories plus issues-list.md); then perform the post-mortem across all eight sections — Progress Assessment, Plan Impact, Decisions Audit, Lessons Learned, Quality Assessment, Delivery Assessment, Updated Project Brief, and Next Milestone Prep — noting any missing QA/DevOps coverage as a gap. Be critical, not encouraging. Write every field in plain language for a human scanning a terminal — full sentences, and expand each issue ID and codename on first mention (per the "Writing clearly" conventions in AGENTS.md and the pm-postmortem presentation layout). You are a NON-INTERACTIVE subagent: do NOT wait for user approval. DRAFT the document updates the skill's Step 3 prescribes — write the updated docs/project-brief.md, update docs/roadmap.md (mark the milestone complete, adjust risks), and append the lessons to .sdlc/lessons-log.md, each written in the same plain "Writing clearly" style (these are durable artifacts future sessions read, not the terminal report) — and list every file you wrote in draftsWritten. Return the full structured assessment.`,
  ...
  return {
    ...
    postmortem: pm,
    nextGate: 'Postmortem gate — present the honest assessment in the pm-postmortem presentation layout (bottom line first, plain language, IDs expanded — see "Writing clearly" in AGENTS.md) along with the drafted doc updates (project-brief, roadmap, lessons-log) for the user to review and approve. After approval, the docs are the record of the closed milestone; write the expert handoff notes (team-milestone Phase 6, step 4) for the experts that did substantive work.',
  }
}
