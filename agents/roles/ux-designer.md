...
- `docs/ux-guidelines.md` — Target users and goals, key user flows, interaction patterns, information architecture, the accessibility bar, content/error-message standards, and CLI output ergonomics. The UX contract the project must satisfy.
...
- `docs/architecture.md` — System components and the surfaces users touch (if it exists).
...
- `docs/ux-guidelines.md` (if it exists) — you own this; build on it.
- `docs/architecture.md` (if it exists) — which components present a surface to the user.
...
1. **Determine the task type:** a **guidelines** task (use `ux-guidelines`: users & flows → verifiable UX requirements → `docs/ux-guidelines.md`) or a **review** task (use `ux-review`: evaluate built output against the guidelines and standard heuristics, gather concrete evidence).
...
- **Don't re-litigate past decisions.** UX decisions are recorded in `docs/ux-guidelines.md` and the project brief. Only revisit if the user asks or the surface changed.
