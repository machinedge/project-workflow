---
name: doc-handoff
description: "End the current Technical Writer session and produce a handoff note. Use when the user signals they're done, wrapping up, or ending the session. Saves session state so the next documentation session can pick up where this one left off."
---
...
Run `.agents/scripts/next-session-number.sh doc` to atomically claim the next session number. The script creates a placeholder file to prevent concurrent sessions from claiming the same number. It outputs the claimed number (e.g., "04").
...
Save to `.sdlc/handoff-notes/doc/session-NN.md`:
...
[MOST IMPORTANT SECTION. Guides that drifted from the code, steps you could not verify, open documentation gaps.]
...
**Handoff note:** `.sdlc/handoff-notes/doc/session-NN.md`
...
If anything was learned during this session (a guide that kept drifting from the code, a deployment step that's easy to get wrong, a recurring readability pitfall), add it to `.sdlc/lessons-log.md`.

Be honest in the handoff note. If a guide is unverified or a step was assumed rather than walked, say so. The next session needs the truth, not optimism.
