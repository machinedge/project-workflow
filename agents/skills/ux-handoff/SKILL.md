---
name: ux-handoff
description: "End the current UX session and produce a handoff note. Use when the user signals they're done, wrapping up, or ending the session. Saves session state so the next UX session can pick up where this one left off."
---
...
Run `.agents/scripts/next-session-number.sh ux` to atomically claim the next session number. The script creates a placeholder file to prevent concurrent sessions from claiming the same number. It outputs the claimed number (e.g., "04").
...
[MOST IMPORTANT SECTION. Open usability risks, unverified flows, blocked fixes.]
...
**Handoff note:** `.sdlc/handoff-notes/ux/session-NN.md`
...
If anything was learned during this session (a recurring usability pitfall, an accessibility gap that's easy to miss, a CLI ergonomics gotcha), add it to `.sdlc/lessons-log.md`.

Be honest in the handoff note. If a review was shallow or a flow is unverified, say so. The next session needs the truth, not optimism.
