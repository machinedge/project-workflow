---
name: ops-pipeline
description: "Design the build/test/deploy pipeline based on environment context. Use when the user needs a pipeline definition that maps their build targets, test infrastructure, and delivery mechanisms into concrete stages with pass/fail criteria."
---
...
If CI config generation is needed, that's an SWE task — create an issue file for it in `.sdlc/issues/backlog/`. Run `.agents/scripts/next-issue-number.sh` for the next number.

Create `.sdlc/issues/backlog/devops-feature-[number].md`:
