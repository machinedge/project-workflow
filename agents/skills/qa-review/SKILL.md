---
name: qa-review
description: "Review code produced by previous SWE sessions with fresh eyes and record findings as issue files. Use when the user wants a code review of completed work, or when evaluating implementation quality against acceptance criteria and architectural intent."
---
...
2. `.sdlc/lessons-log.md` — know what gotchas have already been identified
...
After the user reviews and approves the findings, create issue files in `.sdlc/issues/backlog/` for **Must Fix** and **Should Fix** items. Do NOT create issues for Nits — those are informational only.

Run `.agents/scripts/next-issue-number.sh` to get the next available issue number.

Identify the expert who will fix each finding — use that expert as the filename prefix (executor convention, not finder).

For must-fix findings: `.sdlc/issues/backlog/swe-bug-[number].md`
For should-fix / tech debt findings: `.sdlc/issues/backlog/swe-techdebt-[number].md`
...
- Run `.agents/scripts/update-issues-list.sh` to regenerate `.sdlc/issues/issues-list.md`
- Add any new lessons to `.sdlc/lessons-log.md` (patterns that should be caught earlier next time)
