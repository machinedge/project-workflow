...
Run `.agents/scripts/next-session-number.sh qa` to atomically claim the next session number. The script creates a placeholder file to prevent concurrent sessions from claiming the same number. It outputs the claimed number (e.g., "04").
...
Save to `.sdlc/handoff-notes/qa/session-NN.md`:
...
**Handoff note:** `.sdlc/handoff-notes/qa/session-NN.md`
...
If anything was learned during this session (a gotcha, a review pattern, a quality concern), add it to `.sdlc/lessons-log.md`.
